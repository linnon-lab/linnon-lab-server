...
function buildNotionPageProperties(log, titlePropertyName) {
  ...
  setNumber(properties, "数量", log.quantity ?? null);
...
- calories: 推定摂取カロリー（数値のみ、必ず推定値を入れること。絶対にnullにしない）
- protein: 推定タンパク質量（g、数値のみ、不明ならnull）
- fat: 推定脂質量（g、数値のみ、不明ならnull）
- carbs: 推定炭水化物量（g、数値のみ、不明ならnull）
- sugar: 推定糖質量（g、数値のみ、不明ならnull）
- fiber: 推定食物繊維量（g、数値のみ、不明ならnull）
- salt: 推定塩分量（g、数値のみ、不明ならnull）
- water: 推定水分量（ml、数値のみ、不明ならnull）
- ingredients: 推定材料（改行区切り、不明ならnull）
- recipe: 簡単な調理手順（不明ならnull）
...
app.post("/api/notion/meal-master", async (req, res) => {
  ...
  try {
    ...
    if (data.protein != null) setNumber(properties, "タンパク質", data.protein);
    if (data.fat != null) setNumber(properties, "脂質", data.fat);
    if (data.carbs != null) setNumber(properties, "炭水化物", data.carbs);
    if (data.sugar != null) setNumber(properties, "糖質", data.sugar);
    if (data.fiber != null) setNumber(properties, "食物繊維", data.fiber);
    if (data.salt != null) setNumber(properties, "塩分", data.salt);
    if (data.water != null) setNumber(properties, "水分量", data.water);
